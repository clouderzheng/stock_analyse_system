...
def get_comment(begin_day ,end_day ):
    ...
    for num in range(1):
        ...
        page_begin_month = page_begin_time[0]
        page_begin_day = page_begin_time[1]
        ...
        page_end_time = re.findall("(\d+)月(\d+)日", comment_times[len(comment_times)].text)[0]
        page_end_month = page_begin_time[0]
        page_end_day = page_begin_time[1]
        """最后一条时间大于结束 月份 日期 结束页面爬取 页面是时间倒叙 跳过此次  没有有效的 直接开始下个页面爬取"""
        if(page_end_month > endMonth):
            continue

        if(page_end_day > end_day):
            continue
        ...
        for comment in comments:
            ...
            if(beginMonth > str(comment_lsit[0])):
                break;
            if(beginDay > str(comment_lsit[1])):
                break;
            """评论时间大于选择结束时间  跳过该条 找下一条"""
            if(endMonth < str(comment_lsit[0])):
                continue;
            if(end_day < str(comment_lsit[1])):
                continue;
            ...
            out = open(comment_txt_name,"a+",encoding="utf-8")
